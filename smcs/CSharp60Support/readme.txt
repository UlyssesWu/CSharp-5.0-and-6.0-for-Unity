WARNING: This project's assembly name must be the one from the list below. Otherwise, it won't have
access to UnityEditor's 'internal' types and their 'internal' members.


Possible assembly names:

UnityEditor.Advertisements
Unity.PackageManager
Unity.Automation
Unity.PureCSharpTests
Unity.IntegrationTests
UnityEditor.Graphs
UnityEditor.WP8.Extensions
UnityEditor.Metro.Extensions
UnityEditor.iOS.Extensions
UnityEditor.Xbox360.Extensions
UnityEditor.BB10.Extensions
UnityEditor.Android.Extensions
UnityEditor.Tizen.Extensions
UnityEditor.PS3.Extensions
UnityEditor.SamsungTV.Extensions
UnityEditor.XboxOne.Extensions
UnityEditor.PSP2.Extensions
UnityEditor.PS4.Extensions
UnityEditor.WiiU.Extensions
UnityEditor.PSM.Extensions
UnityEditor.WebGL.Extensions
UnityEditor.LinuxStandalone.Extensions
UnityEditor.WindowsStandalone.Extensions
UnityEditor.OSXStandalone.Extensions
UnityEngine.Networking
UnityEditor.N3DS.Extensions
Unity.RuntimeTests.Framework